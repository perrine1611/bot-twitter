...
# ------- select journalist and tweet ----

def tweet_journalist (client)
...
(1..5).each do
  random = journalists.sample
  client.update ("#{random} Merci pour votre travail ! #bonjour_monde")
end
return random
end
...
def like_tweet(client)
  client.search('#bonjour_monde', result_type: "recent").take(25).each do |tweets|
  client.favorite(tweets)
end
end

# ----------- follow #bonjour_monde

def follow_user(client)
  client.search('#bonjour_monde', result_type: "mixed").take(20).each do |tweet|
...
# ---------- perform 
def perform
  random = tweet_journalist (client)
  client = login_twitter
  tweets = like_tweet(client)
  follow_user(client)
  streaming (client)
end
